...
class ResNet50:
	def __init__(self, uffModelName,batchsize, device,mode,precision,concurrency=1):		
		...
		if precision == "fp16" :
			engine_file_name = "Resnet50_fp16_"
		if precision == "fp32" :
			engine_file_name = "Resnet50_fp32_"
	...
	def predict(self, images, data, max=5, warmup=0, iterations=1):
		...
		resnet50_folder=os.path.join(TRT_DIR,"Results","resnet50")
		if not os.path.isdir(result_folder):
			os.mkdir(result_folder)
		if not os.path.isdir(resnet50_folder):
		...
		if self.mode in ['throughput', 'latency']:
			...
			for i in range(iterations):
				output_text=os.path.join(resnet50_folder,out_text_name)
		...
		if self.mode == "accuracy" :
			...
			output_text_path=os.path.join(TRT_DIR,"Results","resnet50",out_text_name)
