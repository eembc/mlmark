...
def apiRun(params):
	...
	if params[const.PRECISION] == const.FP32:
		net = MobileNet(os.path.join(paths.MODELS, 'tensorflow_lite', model, 'converted_model.tflite'),params[const.HARDWARE])
		return params, common.run(model, net, params)
	if params[const.PRECISION] == const.INT8:
		net = MobileNet(os.path.join(paths.MODELS, 'tensorflow_lite_quantised', model, 'quantised_PTIQ.tflite'),params[const.HARDWARE])
		return params, common.run(model, net, params)
