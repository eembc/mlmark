...
ResNet50_DIR=os.path.join(OPENVINO_DIR,"workloads","resnet50")
...
class ResNet50:
	...
	def predict(self, image_fns, iterations=1000, max=5):
		...
		file_out = os.path.join(ResNet50_DIR,"out.txt")
		...
		batch_folder=ResNet50_DIR+"/batch"+str(self.batch_size)
		...
		result_file_path=ResNet50_DIR+"/result_"+time.strftime('%a%H%M%S')+".txt"
		command = application + " -b "+str(self.batch_size)+" -prec "+self.precision+" -d "+self.device.upper()+" -i "+batch_folder+" -m "+self.modelFilename+" -nt "+str(max)+" -ni "+str(iterations)+" -nireq "+str(self.concurrent_instances)+" -r "+result_file_path+" -mode "+self.mode+" -cl 1000" + " -a " + "resnet50"
