...
std::vector<uint8_t> loadImage_int(string &fn) {
  ...
  data.assign((uint8_t *)img.data, (uint8_t *)img.data + img.total() * 3);
  return data;
}
...
void runBatchInference(vector<string> &filenames) {
  ...
  */
  ...
  armnnTfLiteParser::BindingPointInfo inputBindingInfo =
      parser->GetNetworkInputBindingInfo(0, "input");
  armnnTfLiteParser::BindingPointInfo outputBindingInfo =
      parser->GetNetworkOutputBindingInfo(0,
  ...
  if (g_mode == "throughput" || g_mode == "latency") {
    ...
    for (auto i :
      ...
      for (int j = 0; j < 5 && it != results.rend(); ++j) {
        ...
        prediction_node.put("", it->second - 1);
...
void runBatchInference_int(vector<string> &filenames) {
  ...
  oopts.m_ReduceFp32ToFp16 = false;
  if (g_precision == "int8") {
    // Do nothing, we're using an INT8 model
  } else {
    if (g_precision == "fp16") {
      cout << "Enabling fp16 optimization" << endl;
      oopts.m_ReduceFp32ToFp16 = true;
  ...
  if (g_mode == "throughput" || g_mode == "latency") {
    ...
    for (auto i :
      ...
      vector<float> output_f(1001);
      for (int i = 0; i <= 1001; i++) {
        output_f[i] = float(output[i]) / 255.0;
      }
      map<float, int> results = maxSort(output_f);
